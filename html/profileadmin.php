...
?>
	...
	<table style="width: 100%; text-align: left;" cellpadding='2' cellspacing='2'>
		<tbody>
			<tr>
<?php if ( $state > 1 ) { ?>
				<td class="progressbar"; style="width:<?php echo $state;?>%;"><?php echo $state;?>%</td>
				<td bgcolor=#FFFFFF></td>
<?php } else { ?>
				<td colspan="2" bgcolor=#FFFFFF style="font-size:10pt;text-align:left;width:100%"><?php echo $state;?>%</td>
<?php } ?>
			</tr>
		</tbody>
	</table>
...
<?php	if ( $new_channel ) { ?>
			...
			<th class="CHANNELLIST" colspan="2">
				<input type="text" name="name" id="name" value="<?php echo $channelinfo['name'];?>" 
					style="width:100%">
			</th>
<?php	} else {  ?>
			<th class="CHANNELLIST" colspan="3">
				<?php echo $channelinfo['name'];?>
				<input type="hidden" name="name" id="name" value="<?php echo $channelinfo['name'];?>">
			</th>
			<th class="CHANNELLIST" style="text-align:right" >
				<input type="image" name="delete_channel_commit" value="<?php echo $channelinfo['name'];?>"
...
<?php 	} ?>
		...
		<tr class="CHANNELLIST">
			...
			<td class="MYVALUE" id="colour_cell" style="background-color:<?php echo $channelinfo['colour'];?>" >Enter new value</td>
			<td class="MYVALUE" colspan="2">
				<input type='text' name='colour' id="colour" value='<?php echo $channelinfo['colour']; ?>'  
...
<?php 
	...
	print "SetOrderSelector(1, $order);\n";
	if ( $new_channel ) 
...
function PrintChannelStat ($is_live_profile, $i, $channelinfo, $visible ) {
	global $self;
	...
	<table class="CHANNELLIST" cellpadding="4" cellspacing="4" id="<?php echo $chan_id;?>" >
		...
		<tr class="CHANNELLIST">
			...
			<th class="CHANNELLIST" style="text-align:right" >
				...
				<input type="hidden" name="edit_channel" title="Edit channel" value="<?php echo $chan_name;?>">
				<input type="image" name="edit_channel" title="Edit channel" value="<?php echo $chan_name;?>"
		...
		<tr class="CHANNELLIST" <?php echo $display_style?> id="<?php echo $chan_id . '_1';?>">
			<td class="CHANNELLIST">Colour:</td>
			<td class="MYVALUE" style="background-color:<?php echo $channelinfo['colour'];?>" >
				<?php echo $channelinfo['colour'];?></td>
			<td class="CHANNELLIST">Sign:</td><td class="MYVALUE"><?php echo $channelinfo['sign'];?></td>
			<td class="CHANNELLIST">Order:</td><td class="MYVALUE"><?php echo $channelinfo['order'];?></td>
		</tr>
...
<?php 
	...
	for ( $j=0; $j < count($sourcelist); $j++ ) {
		print "<option>" . $sourcelist[$j] . "</option>\n";
	}
?>
				</select>
			</td>
...
<?php if ( $opt_delete ) { ?>
	<form style="display:inline;" name="profileform" id="profileform" action="<?php  echo $self;?>" method="POST" 
		onSubmit="return ConfirmDeleteProfile(<?php print '\'' . $_SESSION['profile'] . '\', \'' . $_SESSION['profilegroup'] . '\''; ?>);">
    	<input type="image" name="deleteprofile" value="<?php echo $_SESSION['profileswitch']?>" title="Delete this profile" src="icons/trash.png" align="right" >
		<input type="hidden" name="switch" value="<?php echo $_SESSION['profileswitch']?>" >
	</form>
<?php } if ( $opt_cancel ) { ?>
	<form style="display:inline;" name="profileform" id="profileform" action="<?php  echo $self;?>" method="POST" 
		onSubmit="return ConfirmCancelBuild(<?php print '\'' . $_SESSION['profile'] . '\', \'' . $_SESSION['profilegroup'] . '\''; ?>);">
    	<input type="image" name="cancelbuild" value="<?php echo $_SESSION['profileswitch']?>" title="Cancel building profile" src="icons/cancel.png" align="right" >
		<input type="hidden" name="switch" value="<?php echo $_SESSION['profileswitch']?>" >
	</form>
<?php }
...
<?php } 
		print $_SESSION['profilegroup'] == '.' ? "(nogroup)" : $_SESSION['profilegroup'] ;
?>
	</td>
</tr>
...
<tr class="STATTABLE" id="ed_description_ro">
    ...
    <td class="MYVALUE">
		...
		<tr>
			<td>
				<textarea name="description" rows="4" cols="40" style="width:100%" readonly ><?php  echo stripslashes($description); ?></textarea>
			</td>
...
<tr class="STATTABLE">
    ...
    <td class="MYVALUE"> <?php  echo $size; ?> </td>
</tr>
...
<?php } ?>
		<input type="text" name="profile_maxsize" value="<?php echo $maxsize; ?>" SIZE="16" MAXLENGTH="16" 
			style="font-size:12px;" readonly>
    </td>
</tr>
...
<?php } ?>
		<input type="text" name="profile_expire" value="<?php  echo $expire; ?>" SIZE="16" MAXLENGTH="16" style="font-size:12px;" readonly >
    </td>
</tr>
...
<?php } ?>
	...
	foreach ( $_SESSION['profileinfo']['channel'] as $channel ) {
		...
		PrintChannelStat($is_live_profile, $i, $channel, $_COOKIE['extended_channellist'] );
		print "</td>\n";
		print "</tr>\n";
		$i++;
	}
...
<tr class="STATTABLE">
    ...
    <td class="MYVALUE">
		<input type="text" name="profile" id="profile" value="<?php echo $new_profile['profile']; ?>"
...
<tr class="STATTABLE">
    ...
    <td class="MYVALUE">
    	<textarea name="description" cols="25" rows="3" 
			style="width:100%" ><?php  echo stripslashes($description); ?></textarea>
    </td>
	<td>
	</td>
</tr>
<tr class="STATTABLE">
    <td class="STATTABLE">Start:</td> 
    <td class="MYVALUE">
		<input type="text" name="tstart" value="<?php echo $new_profile['tstart']; ?>"
...
<tr class="STATTABLE">
    <td class="STATTABLE">End:</td> 
    <td class="MYVALUE">
		<input type="text" name="tend" value="<?php echo $new_profile['tend']; ?>"
			SIZE="17" MAXLENGTH="16" style="font-size:12px;">&nbsp;<b>Format: yyyy-mm-dd-HH-MM</b>
	</td>
	<td>
...
<tr class="STATTABLE">
    <td class="STATTABLE">Max. Size:</td> 
    <td class="MYVALUE">
		<input type='text' name='maxsize' value="<?php echo $new_profile['maxsize']; ?>" SIZE='16' MAXLENGTH='16' 
...
<tr class="STATTABLE">
	...
	<?php  $_tmp = ExpireString($new_profile['expire']); ?>
		<input type='text' name='expire' value="<?php  echo $_tmp; ?>" SIZE='16' MAXLENGTH='16' 
			style="font-size:12px;" >
	</td>
	<td>
...
<?php
	foreach ( $sources as $source ) {
		$selected = array_key_exists($source, $selected_channel) ? "selected" : '';
...
<tr class="STATTABLE" id="filter_element" style="display: none">
    ...
    <td class="MYVALUE">
    	<textarea name="filter" cols="25" rows="3" style="width:100%" ><?php  echo stripslashes($filter); ?></textarea>
    </td>
	<td>
...
<script>
	...
	ChannelWizard('<?php echo $new_profile['channel_wizard'];?>');
...
function Process_stat_tab ($tab_changed, $profile_changed) {
	...
	if ( array_key_exists('regroup', $_POST ) ) {
		...
		$_group = $_POST['groupselect'];
		if ( $_group == '(nogroup)' ) {
			$_group = '.';
		} else if ( $_group == 'New group ...' ) {
			$_group = $_POST['profilegroup'];
		}
